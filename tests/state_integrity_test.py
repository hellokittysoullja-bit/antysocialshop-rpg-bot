...
async def check_lab_run_survives_reload_and_pays_once(repo, pool):
    ...
    # atomic_finish_lab_run обязан считать дельту total_earned так же, как
    # atomic_update (единая точка учёта заработка) — иначе сундук/утешительный
    # приз пополняет кошелёк, но не двигает ранг/статус. Раньше этой проверки
    # не было вообще: тест сверял только balance, регрессия проходила молча.
    winner = await repo.get_by_id(2001)
    assert winner.total_earned == 215, (
        "выплата Лабиринта обязана попасть в total_earned (иначе ранг не растёт "
        f"от сундука): total_earned={winner.total_earned}, balance={winner.balance}")
